					...
					<Card key={d.id} className="mb-4">
						<CardContent className="flex flex-col p-5">
							<div className="flex flex-row justify-between align-middle pb-4">
								<span><Icon variant="red" >{StatIcon && <StatIcon  />} </Icon></span>
								<span><Pill variant="secondary">{d.status}</Pill></span>
							</div>
							<div className="flex flex-col">
								<span className="text-sm capitalize">{d.type.replace("_", " ")}</span>
								<span><span className="text-3xl font-semibold text-foreground mr-2" >{d.value}</span>{d.unit}</span>
								<span className="text-xs pt-2">Last updated:{format(new Date(d.date),"MMM dd, yyy")}</span>
							</div>
